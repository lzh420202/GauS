...
@register_box('rrbox')
class RRotatedBoxes(RotatedBoxes):
    '''
    Real Rotated Box, angle define: [0, 360)
    '''
    def regularize_boxes(self,
                         pattern: Optional[str] = None,
                         width_longer: bool = True,
    ...
    def flip_(self,
        ...
        if direction == 'horizontal':
            flipped[..., 0] = img_shape[1] - flipped[..., 0]
            flipped[..., 4] = torch.where(np.pi - flipped[..., 4] < 0,
                                          3.0 * np.pi - flipped[..., 4],
                                          np.pi - flipped[..., 4])
            # flipped[..., 4] = -flipped[..., 4]
        elif direction == 'vertical':
            flipped[..., 1] = img_shape[0] - flipped[..., 1]
            # flipped[..., 4] = -flipped[..., 4]
            flipped[..., 4] = 2.0 * np.pi - flipped[..., 4]
        else:
            flipped[..., 0] = img_shape[1] - flipped[..., 0]
            flipped[..., 1] = img_shape[0] - flipped[..., 1]
            flipped[..., 4] = (np.pi + flipped[..., 4]) % (2.0 * np.pi)
    ...
    def rbox2corner(boxes: Tensor) -> Tensor:
        ...
        ctr, w, h, theta = torch.split(boxes, (2, 1, 1, 1), dim=-1)
        cos_value, sin_value = torch.cos(theta), torch.sin(theta)
        vec1 = torch.cat([w / 2.0 * cos_value, w / 2.0 * sin_value], dim=-1) # direction
        vec2 = torch.cat([h / 2.0 * sin_value, -h / 2.0 * cos_value], dim=-1) # theta - 0.5 * pi
        pt1 = ctr + vec1 + vec2 # top left
        pt2 = ctr + vec1 - vec2 # top right
        pt3 = ctr - vec1 - vec2
        pt4 = ctr - vec1 + vec2
        return torch.stack([pt1, pt2, pt3, pt4], dim=-2)
    ...
    def corner2rbox(corners: Tensor) -> Tensor:
        ...
        direction = (head_points - ct_pionts) / np.sqrt(
            np.sum(np.power((head_points - ct_pionts), 2.0), axis=1, keepdims=True))
        rboxes = []
    ...
    def rescale_(self, scale_factor: Tuple[float, float]) -> None:
        ...
        h = h * torch.sqrt((scale_x * sin_value)**2 + (scale_y * cos_value)**2)
        # recalculate theta
        t = torch.atan2(scale_x * sin_value, scale_y * cos_value) % (2.0 * np.pi)
        self.tensor = torch.cat([ctrs, w, h, t], dim=-1)
    ...
    def overlaps(boxes1: BaseBoxes,
        ...
        """
        from mmrotate.structures.bbox import rbbox_overlaps
        boxes1 = boxes1.convert_to('rrbox')
        boxes2 = boxes2.convert_to('rrbox')
        return rbbox_overlaps(
            boxes1.tensor,
            boxes2.tensor,
            mode=mode,
    ...
    @staticmethod
    def from_instance_masks(masks: MaskType) -> 'RRotatedBoxes':
        """Create boxes from instance masks.
        ...
        num_masks = len(masks)
        if num_masks == 0:
            return RRotatedBoxes([], dtype=torch.float32)
