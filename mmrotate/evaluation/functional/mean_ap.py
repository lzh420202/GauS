...
def tpfp_cos(det_bboxes,
    ...
    tp = np.zeros((num_scales, num_dets), dtype=np.float32)
    fp = np.zeros((num_scales, num_dets), dtype=np.float32)
    tp_c = np.zeros((num_scales, num_dets), dtype=np.float32)
    fp_c = np.zeros((num_scales, num_dets), dtype=np.float32)
        ...
        for i in sort_inds:
            if ious_max[i] >= iou_thr:
                ...
                if not (gt_ignore_inds[matched_gt]
                    ...
                    if not gt_covered[matched_gt]:
                        gt_covered[matched_gt] = True
                        orient_match_rate = np.cos(gt_angles[matched_gt] - det_angles[i])
                        if orient_match_rate <= 0:
                            fp_c[k, i] = 1
                        else:
                            tp_c[k, i] = orient_match_rate
                        tp[k, i] = 1
                    else:
                        fp[k, i] = 1
                        fp_c[k, i] = 1
...
def eval_rrbbox_map(det_results,
                    annotations,
                    scale_ranges=None,
                    iou_thr=0.5,
                    use_07_metric=True,
    ...
    for i in range(num_classes):
        ...
        tpfp = pool.starmap(
            ...
            zip(cls_dets, cls_gts, cls_gts_ignore,
                ...
                [box_type for _ in range(num_imgs)],
                [area_ranges for _ in range(num_imgs)]))
        tp, fp, tp_c, fp_c = tuple(zip(*tpfp))
        # calculate gt number of each scale
        # ignored gts or gts beyond the specific scale are not counted
        num_gts = np.zeros(num_scales, dtype=int)
        for _, bbox in enumerate(cls_gts):
        ...
        tp_c = np.hstack(tp_c)[:, sort_inds]
        fp_c = np.hstack(fp_c)[:, sort_inds]
        # calculate recall and precision with tp and fp
        tp_c_m = (tp_c > 0).astype(np.float32)
        tp_c_m = np.cumsum(tp_c_m, axis=1)
        tp_c = np.cumsum(tp_c, axis=1)
        fp_c = np.cumsum(fp_c, axis=1)
        recalls_c = tp_c / np.maximum(num_gts[:, np.newaxis], eps)
        precisions_c = tp_c / np.maximum((tp_c_m + fp_c), eps)

        # calculate AP
        if scale_ranges is None:
            recalls = recalls[0, :]
            precisions = precisions[0, :]
            recalls_c = recalls_c[0, :]
            precisions_c = precisions_c[0, :]
            num_gts = num_gts.item()
        mode = 'area' if not use_07_metric else '11points'
        ap = average_precision(recalls, precisions, mode)
        ap_c = average_precision(recalls_c, precisions_c, mode)
        eval_results.append({
            'num_gts': num_gts,
            'num_dets': num_dets,
            'recall': recalls,
    ...
    if scale_ranges is not None:
        # shape (num_classes, num_scales)
        all_ap = np.vstack([cls_result['ap'] for cls_result in eval_results])
        all_ap_c = np.vstack([cls_result['ap_c'] for cls_result in eval_results])
        all_num_gts = np.vstack(
            [cls_result['num_gts'] for cls_result in eval_results])
        mean_ap = []
        mean_ap_c = []
        for i in range(num_scales):
            if np.any(all_num_gts[:, i] > 0):
                mean_ap.append(all_ap[all_num_gts[:, i] > 0, i].mean())
                mean_ap_c.append(all_ap_c[all_num_gts[:, i] > 0, i].mean())
            else:
                mean_ap.append(0.0)
                mean_ap_c.append(0.0)
    else:
        aps = []
        aps_c = []
        for cls_result in eval_results:
            if cls_result['num_gts'] > 0:
                aps.append(cls_result['ap'])
                aps_c.append(cls_result['ap_c'])
        mean_ap = np.array(aps).mean().item() if aps else 0.0
        mean_ap_c = np.array(aps_c).mean().item() if aps_c else 0.0

    if show:
        print_map_summary_c(
            mean_ap, mean_ap_c, eval_results, dataset, area_ranges, logger=logger)

    return mean_ap, mean_ap_c, eval_results
...
def print_map_summary_c(mean_ap,
                        mean_ap_c,
                        results,
                        dataset=None,
                        scale_ranges=None,
    ...
    recalls = np.zeros((num_scales, num_classes), dtype=np.float32)
    aps = np.zeros((num_scales, num_classes), dtype=np.float32)
    aps_c = np.zeros((num_scales, num_classes), dtype=np.float32)
    num_gts = np.zeros((num_scales, num_classes), dtype=int)
    for i, cls_result in enumerate(results):
        if cls_result['recall'].size > 0:
            recalls[:, i] = np.array(cls_result['recall'], ndmin=2)[:, -1]
        aps[:, i] = cls_result['ap']
        aps_c[:, i] = cls_result['ap_c']
        num_gts[:, i] = cls_result['num_gts']
    ...
    if not isinstance(mean_ap, list):
        mean_ap = [mean_ap]
    if not isinstance(mean_ap_c, list):
        mean_ap_c = [mean_ap_c]

    header = ['class', 'gts', 'dets', 'recall', 'ap', 'ap_c']
    for i in range(num_scales):
        if scale_ranges is not None:
            print_log(f'Scale range {scale_ranges[i]}', logger=logger)
        table_data = [header]
        for j in range(num_classes):
            row_data = [
                label_names[j], num_gts[i, j], results[j]['num_dets'],
                f'{100.0 * recalls[i, j]:.2f}',
                f'{100.0 * aps[i, j]:.2f}',
                f'{100.0 * aps[i, j]:.2f}'
            ]
            table_data.append(row_data)
        table_data.append(['mAP', '', '', '', f'{100.0 * mean_ap[i]:.2f}', f'{100.0 * mean_ap_c[i]:.2f}'])
        table = AsciiTable(table_data)
        table.inner_footing_row_border = True
        print_log('\n' + table.table, logger=logger)