...
from pre_process import imageStetch
...
def add_parser(parser):
    ...
    parser.add_argument(
        '--pre-process',
        action='store_true',
        help='pre process switch')
    parser.add_argument(
        '--pre-process-stetch',
        type=float,
        default=0.05,
        help='stetch threshold')
...
def crop_and_save_img(info, windows, window_anns, img_dir, no_padding,
                      padding_value, save_dir, anno_dir, img_ext, pre_process):
    """
    ...
    if pre_process is not None:
        img = cv2.imread(osp.join(img_dir, info['filename']), cv2.IMREAD_UNCHANGED)
    else:
        img = cv2.imread(osp.join(img_dir, info['filename']))
    if pre_process is not None:
        assert isinstance(pre_process, float)
        assert (pre_process > 0.0) & (pre_process < 1.0)
        img = imageStetch(img, pre_process)
...
def _load_dota_single(imgfile, img_dir, ann_dir):
    ...
    if ext not in ['.jpg', '.JPG', '.png', '.tif', '.tiff', '.TIFF', '.bmp']:
        return None
...
def main():
    ...
    if args.pre_process:
        pre_process = args.pre_process_stetch
        print(f'\033[1;32m pre-process:ON\nstetch: {pre_process}\033[0m')
    else:
        pre_process = None
        print(f'\033[1;31m pre-process:OFF\033[0m')
