...
from mmrotate.evaluation import eval_rbbox_map, eval_rrbbox_map
...
class DOTAMetric(BaseMetric):
    ...
    def compute_metrics(self, results: list) -> dict:
        ...
        if self.metric == 'mAP':
            ...
            orders = []
            summary_str = ''
            for iou_thr in tqdm(self.iou_thrs):
                # logger.info(f'\n{"-" * 15}iou_thr: {iou_thr}{"-" * 15}')
                mean_ap, all_result = eval_rbbox_map(
                    dets,
                    gts,
                ...
                order = f'AP{int(iou_thr * 100):02d}'
                eval_results[order] = mean_ap
                orders.append(order)
                for i, name in enumerate(dataset_name):
                    if eval_results.get(name, None) is None:
                        eval_results[name] = dict()
                    ap = float(all_result[i]['ap'])
                    eval_results[name][order] = ap
                    if order == 'AP50':
                        summary_str += f'{name}: {ap}, '
            eval_results['mAP'] = sum(mean_aps) / len(mean_aps)
            # summary_str += f"AP50:{eval_results['AP50']}, " \
            #                f"AP75:{eval_results['AP75']}, " \
            #                f"mAP:{eval_results['mAP']}."
            try:
                add_str = f"AP75:{eval_results['AP75']}, "
            except:
                add_str = ""
            summary_str += f"AP50:{eval_results['AP50']}, " \
                           f"{add_str}" \
                           f"mAP:{eval_results['mAP']}."
            eval_results['summary'] = summary_str
            for order in reversed(orders):
                eval_results.move_to_end(order, last=False)
            eval_results.move_to_end('mAP', last=False)
            eval_results.move_to_end('summary', last=False)
        else:
            raise NotImplementedError
        return eval_results
        ...
        if self.merge_patches:
            ...
            if self.format_only:
                ...
                logger.info('results are saved in '
                            f'{osp.dirname(outfile_prefix)}')
                return eval_results
        eval_results['AP'] = OrderedDict()
        eval_results['AP_c'] = OrderedDict()
        if self.metric == 'mAP':
            assert isinstance(self.iou_thrs, list)
            dataset_name = self.dataset_meta['classes']
            dets = [pred['pred_bbox_scores'] for pred in preds]
            ...
            for iou_thr in tqdm(self.iou_thrs):
                # logger.info(f'\n{"-" * 15}iou_thr: {iou_thr}{"-" * 15}')
                mean_ap, mean_ap_c, all_result = eval_rrbbox_map(
                    dets,
                    gts,
                    scale_ranges=self.scale_ranges,
                    iou_thr=iou_thr,
                    ...
                    logger=logger)
                mean_aps.append(mean_ap)
                mean_aps_c.append(mean_ap_c)
                order = f'AP{int(iou_thr * 100):02d}'
                eval_results['AP'][order] = mean_ap
                eval_results['AP_c'][order] = mean_ap_c
                orders.append(order)
                for i, name in enumerate(dataset_name):
                    if eval_results['AP'].get(name, None) is None:
                        eval_results['AP'][name] = dict()
                    if eval_results['AP_c'].get(name, None) is None:
                        eval_results['AP_c'][name] = dict()
                    ap = float(all_result[i]['ap'])
                    ap_c = float(all_result[i]['ap_c'])
                    eval_results['AP'][name][order] = ap
                    eval_results['AP_c'][name][order] = ap_c
                    if order == 'AP50':
                        summary_str += f'AP# {name}: {ap}, '
                        summary_str_c += f'AP_c# {name}: {ap_c}, '
            eval_results['AP']['mAP'] = sum(mean_aps) / len(mean_aps)
            eval_results['AP_c']['mAP'] = sum(mean_aps_c) / len(mean_aps_c)
            # summary_str += f"AP50:{eval_results['AP50']}, " \
            #                f"AP75:{eval_results['AP75']}, " \
            #                f"mAP:{eval_results['mAP']}."
            try:
                add_str = f"AP75:{eval_results['AP']['AP75']}, "
                add_str_c = f"AP75:{eval_results['AP_c']['AP75']}, "
            except:
                add_str = ""
                add_str_c = ''
            summary_str += f"AP50:{eval_results['AP']['AP50']}, " \
                           f"{add_str}" \
                           f"mAP:{eval_results['AP']['mAP']}."
            summary_str_c += f"AP50:{eval_results['AP_c']['AP50']}, " \
                             f"{add_str_c}" \
                             f"mAP:{eval_results['AP_c']['mAP']}."
            eval_results['AP']['summary'] = summary_str
            eval_results['AP_c']['summary'] = summary_str_c
            for order in reversed(orders):
                eval_results['AP'].move_to_end(order, last=False)
                eval_results['AP_c'].move_to_end(order, last=False)
            eval_results['AP'].move_to_end('mAP', last=False)
            eval_results['AP'].move_to_end('summary', last=False)
            eval_results['AP_c'].move_to_end('mAP', last=False)
            eval_results['AP_c'].move_to_end('summary', last=False)
        else:
            raise NotImplementedError
        return eval_results