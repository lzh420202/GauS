...
import mmcv
import numpy as np
from mmcv.transforms import BaseTransform, RandomChoiceResize, Resize
...
@TRANSFORMS.register_module()
class ReResize(Resize):

    def _resize_bboxes(self, results: dict) -> None:
        """Resize bounding boxes with ``results['scale_factor']``."""
        if results.get('gt_bboxes', None) is not None:
            # bboxes = results['gt_bboxes'] * np.concatenate([np.tile(
            #     np.array(results['scale_factor']), 2), np.array([1.0, ], dtype=np.float)])
...
@TRANSFORMS.register_module()
class RandomChoiceReResize(RandomChoiceResize):
    pass
    # def __init__(
    #     self,
