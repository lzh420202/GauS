...
class DOTADataset(BaseDataset):
        ...
        if self.ann_file == '':
            ...
            if len(txt_files) == 0:
                raise ValueError('There is no txt file in '
                                 f'{self.ann_file}')
            # data_cache = osp.join(os.path.dirname(self.ann_file), 'dump_anno.pkl')
            data_cache = osp.abspath(osp.join(self.ann_file, '..', 'dump_anno.pkl'))
    ...
    def filter_data(self) -> List[dict]:
        ...
        valid_data_infos = []
        for i, data_info in tqdm(enumerate(self.data_list)):
