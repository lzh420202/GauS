...
from tool import merge_results, DumpMergeResults
...
def parse_args():
    ...
    parser.add_argument(
        ...
        type=str,
        default=None,
        help='dump metric result to a json file')
    parser.add_argument(
        '--tta',
        action='store_true',
        default=False,
...
def main():
    ...
    if args.merge_out is not None:
        assert args.merge_out.endswith(('.pkl', '.pickle')), \
            'The dump file must be a pkl file.'
        import copy
        ...
        runner.test_evaluator.metrics.append(
            DumpMergeResults(out_file_path=args.merge_out,
    ...
    for key in result.keys():
        if isinstance(result[key], Dict):
            for k in result[key].keys():
                if isinstance(result[key][k], np.float32):
                    result[key][k] = float(result[key][k])

    if args.metric_out is not None:
        import json
